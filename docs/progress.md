...
| API nghiệp vụ | 99% | Đã có API public/protected cho dashboard, form, danh mục public, kết quả public, lịch sử public, lỗi nguy cơ cao public, báo cáo public, CAPA public, lịch/phiên kiểm tra public, phân công public, chấm điểm, upload minh chứng, CAPA update, chốt/mở khóa kỳ, tạo phiên kiểm tra, tạo phân công, import prepare/commit và export Excel qua backend; audit log đã được map đúng cột database. |
...
2. Mở rộng upload minh chứng cho CAPA riêng nếu cần tách khỏi minh chứng điểm chấm.
3. Bổ sung vùng chữ ký/logo nhúng trong Excel nếu chuyển sang thư viện hỗ trợ style đầy đủ.
