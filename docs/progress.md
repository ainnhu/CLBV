...
| Database và bảo mật | 78% | Đã bổ sung schema Supabase theo hướng public-read/protected-write, thêm `report_files`, `public_profiles`, index, grant cho `anon/authenticated`, RLS cho hồ sơ cá nhân, phân công, report file và thao tác quản trị. |
| Auth/API quyền | 86% | API protected đã hỗ trợ demo header và có thể đọc Supabase Bearer token khi cấu hình thật; request ghi vẫn kiểm tra quyền backend trước khi xử lý body/file. |
| API nghiệp vụ | 86% | Đã có API public/protected cho dashboard, form, chấm điểm, CAPA, chốt/mở khóa kỳ, import prepare/commit và export Excel; audit log đã được map đúng cột database. |
...
1. Tạo seed SQL/dữ liệu khởi tạo cho Supabase: khoa/phòng, đoàn kiểm tra, user mẫu, phân công, kỳ kiểm tra.
2. Hoàn thiện repository đọc dashboard từ Supabase thay vì chỉ mock data.
3. Hoàn thiện API tạo phiên kiểm tra và tạo `inspection_forms` từ `form_templates`.
4. Hoàn thiện màn hình rà soát import Excel trước khi commit chính thức.
5. Hoàn thiện export Excel lưu file vào Supabase Storage, rồi ghi metadata vào `report_exports` và `report_files`.
6. Viết hướng dẫn chạy Supabase từng bước cho người không chuyên CNTT.
7. Sau khi lõi ổn, quay lại tinh giản giao diện chấm điểm mobile-first và dashboard.
