...
| Database và bảo mật | 84% | Đã bổ sung schema Supabase theo hướng public-read/protected-write, thêm `report_files`, `public_profiles`, `inspection_assignments.created_at`, policy đọc công khai metadata minh chứng điểm chấm, index, grant cho `anon/authenticated`, RLS cho hồ sơ cá nhân, phân công, report file và thao tác quản trị. Có migration `database/migrations/20260527_core_fixes.sql` cho database đã chạy schema cũ. Đã có API health protected để kiểm trạng thái cấu hình Supabase/Storage mà không lộ khóa bí mật. |
| Auth/API quyền | 94% | API protected đã hỗ trợ demo header và có thể đọc Supabase Bearer token khi cấu hình thật; request ghi kiểm tra quyền backend trước khi xử lý body/file, dùng Zod schema cho dữ liệu chấm điểm, CAPA update, phiên kiểm tra, phân công, commit import Excel và quản trị danh mục; JSON hỏng trả `400`, dữ liệu sai nghiệp vụ trả `422`; health/config chỉ Admin/Phòng KHTH xem được. |
| API nghiệp vụ | 99% | Đã có API public/protected cho dashboard, form, danh mục public, kết quả public, lịch sử public, lỗi nguy cơ cao public, báo cáo public, CAPA public, lịch/phiên kiểm tra public, phân công public, chấm điểm, upload minh chứng điểm chấm, upload minh chứng CAPA, CAPA update, quản trị khoa/phòng và đoàn kiểm tra, chốt/mở khóa kỳ, tạo phiên kiểm tra, tạo phân công theo quyền `assignment:manage`, import prepare/commit và export Excel qua backend; audit log đã được map đúng cột database. |
| Import Excel | 84% | Parser ưu tiên sheet phiếu kiểm tra/chấm điểm theo khoa/phòng, tạo import batch UUID, cảnh báo lệch số tiêu chí/tổng điểm, có màn hình đọc thử/rà soát cảnh báo trước khi commit `upsert_version`; commit import đã validate cấu trúc phiếu/tiêu chí tối thiểu và chặn ghi nếu còn cảnh báo chưa được cho phép. |
...
- Thao tác ghi như chấm điểm, cập nhật CAPA, chốt/mở khóa kỳ, import Excel, commit import, quản trị danh mục, tạo phiên, phân công và export báo cáo đều kiểm tra quyền ở API/backend.
...
- Local `node scripts/smoke-test.mjs http://localhost:3114`: đạt `32/32`.
- Vercel `node scripts/smoke-test.mjs https://clbv.vercel.app`: đạt `32/32`.
...
- Local API protected commit import: không đăng nhập `403`, dữ liệu phiếu/tiêu chí rỗng `422`, còn cảnh báo nhưng chưa cho phép `422`, payload hợp lệ bằng `Admin` `200`.
- Local API protected system health: không đăng nhập `403`, vai trò `Admin` `200`, có `summary/supabase/checks`.
...
- Local API protected sessions: tạo không đăng nhập `403`, JSON hỏng bằng `Admin` `400`, dữ liệu sai `422`, tạo phiên bằng `Admin` `200`.
- Local API protected assignments: tạo không đăng nhập `403`, JSON hỏng bằng `Admin` `400`, dữ liệu sai `422`, tạo phân công bằng `Admin` `200`.
