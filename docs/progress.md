...
| Auth/API quyền | 90% | API protected đã hỗ trợ demo header và có thể đọc Supabase Bearer token khi cấu hình thật; request ghi kiểm tra quyền backend trước khi xử lý body/file, dùng Zod schema cho dữ liệu chấm điểm, CAPA update và quản trị danh mục; JSON hỏng trả `400`, dữ liệu sai nghiệp vụ trả `422`. |
| API nghiệp vụ | 99% | Đã có API public/protected cho dashboard, form, danh mục public, kết quả public, lịch sử public, lỗi nguy cơ cao public, báo cáo public, CAPA public, lịch/phiên kiểm tra public, phân công public, chấm điểm, upload minh chứng điểm chấm, upload minh chứng CAPA, CAPA update, quản trị khoa/phòng và đoàn kiểm tra, chốt/mở khóa kỳ, tạo phiên kiểm tra, tạo phân công, import prepare/commit và export Excel qua backend; audit log đã được map đúng cột database. |
...
- Thao tác ghi như chấm điểm, cập nhật CAPA, chốt/mở khóa kỳ, import Excel, commit import, quản trị danh mục và export báo cáo đều kiểm tra quyền ở API/backend.
