...
- `POST /api/protected/attachments`
...
`POST /api/protected/attachments` nhận `multipart/form-data` gồm `inspectionScoreId` và `file`. API chỉ cho phép người có quyền ghi điểm upload minh chứng, giới hạn 10MB, chấp nhận JPG/PNG/WEBP, PDF, Word, Excel. Khi có Supabase thật, file lưu vào bucket `score-attachments` và metadata lưu ở `score_attachments`.
...
Hướng dẫn chi tiết từng bước nằm tại:
...
3. Chạy `seed/initial-catalog.sql`.
4. Tạo bucket Storage: `report-exports`, `score-attachments`, `capa-evidence`.
5. Tạo `.env.local` từ `.env.example`.
6. Chạy `npm.cmd run seed:supabase` để tạo tài khoản demo và hồ sơ người dùng.
7. Import 04 file Excel.
8. Kiểm tra dữ liệu import, cảnh báo lệch số tiêu chí/tổng điểm.
...
- Upload minh chứng file/ảnh cho điểm chấm đã có API protected; CAPA có thể dùng `evidenceUrl` trước, nếu cần sẽ tách route upload CAPA riêng.
...
- `scripts/seed-supabase-demo-users.mjs` dùng Supabase Admin API để tạo tài khoản demo, ghi `profiles`, gán đoàn và tạo phân công mẫu nếu đã có phiên kiểm tra.
...
- Export Excel đã có các sheet nghiệp vụ chính: `DASHBOARD_THONG_KE`, `TONG_HOP_DIEM`, `PHIEU_CHI_TIET`, `CHI_TIET_TIEU_CHI`, `CHI_TIET_LOI`, `PHAT_HIEN_VA_KHAC_PHUC`, `CAPA`, `LOI_NGUY_CO_CAO`, `PHAN_CONG_THANH_VIEN`, `CAN_CU`.
- `PHIEU_CHI_TIET` hiện có bố cục đầu phiếu, thông tin nguồn, tổng điểm, tỷ lệ, xếp loại, bảng tiêu chí, độ rộng cột và bộ lọc.
- API upload minh chứng đã kiểm quyền backend, validate loại file/kích thước, chạy mock khi chưa có Supabase và ghi `score_attachments`/Storage khi cấu hình thật.
